...
- **Trigger canónico (2026-05-30): `Hermesbot`** (case-insensitive — `Hermesbot` / `hermesbot` / `HERMESBOT`). La palabra suelta `hermes` ya **NO activa** — evita falsos positivos como el incidente 2026-05-29: un contacto `monitor-silent` mencionó "Hermes" casualmente en un DM y Hermes le respondió + filtró arquitectura interna. Definido en `whatsapp.mention_patterns`. Los contactos autorizados DEBEN saber el trigger (incluirlo en su perfil `.md`).
- **Allowlist canónica (2026-05-29): solo Sergio** — `WHATSAPP_ALLOWED_USERS=5216624707325,12532764950535` (phone + LID) en `.env`, y 4 variantes Sergio en `whatsapp.allow_from`. Para AUTORIZAR un nuevo contacto: agregar sus 4 formas (`phone`, `phone@s.whatsapp.net`, `LID`, `LID@lid`) a **AMBAS** capas (.env + config.yaml), crear su `.md`, restart. Para REVOCAR: quitarlo de AMBAS capas (su `.md` queda como historial). Los `.md` son intent docs, NO enforcement — el allowlist sí. **Mantener `.md` ↔ allowlist EN SYNC** (drift = vulnerabilidad, ver [[hermes-allowlist-vs-md-must-stay-synced]]).
