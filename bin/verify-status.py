...
def claims() -> list[Claim]:
    ...
    return [
        ...
        Claim("§1", "Hermes gateway activo (systemd hermes-gateway.service)",
              "systemctl --user is-active hermes-gateway.service",
              lambda: ((s := sh("systemctl --user is-active hermes-gateway.service 2>&1").strip()),
                       (s == "active", s if s else "MISSING"))[1]),
        ...
        Claim("§1", "Hermes fallback chain configurado (resiliencia)",
              "~/.hermes/config.yaml fallback_providers",
              lambda: ((c := hermes_config()),
                       ("fallback_providers:" in c and "fallback_providers: []" not in c,
                        "chain presente" if ("fallback_providers:" in c and "fallback_providers: []" not in c) else "sin fallbacks"))[1]),
        ...
        Claim("§2/§11", "OpenClaw dmPolicy = 'disabled' (canónico: bypass direct['*'] cerrado, sin self-DM leak)",
              "openclaw.json channels.whatsapp.dmPolicy",
              lambda: ((openclaw_json().get("channels", {}).get("whatsapp", {}).get("dmPolicy") == 'disabled'),
                       str(openclaw_json().get("channels", {}).get("whatsapp", {}).get("dmPolicy")))),
        Claim("§2/§11", "OpenClaw allowFrom = [] (defense-in-depth: aún si dmPolicy se reabre por error)",
              "openclaw.json channels.whatsapp.allowFrom",
              lambda: ((openclaw_json().get("channels", {}).get("whatsapp", {}).get("allowFrom") == []),
        ...
        Claim("§7/§16", "sync-dms-to-gbrain.py existe + ejecutable (Hermes DMs → GBrain)",
              "ls -la ~/whatsapp-monitor/bin/sync-dms-to-gbrain.py",
              lambda: ((os.access(str(HOME / "whatsapp-monitor/bin/sync-dms-to-gbrain.py"), os.X_OK)),
                       "ejecutable" if os.access(str(HOME / "whatsapp-monitor/bin/sync-dms-to-gbrain.py"), os.X_OK) else "MISSING/no-exec")),
        Claim("§7/§16", "Cron sync-dms-to-gbrain instalado (cada 15 min)",
              "crontab -l | grep sync-dms-to-gbrain",
              lambda: ((os.popen("crontab -l 2>/dev/null | grep -c sync-dms-to-gbrain").read().strip() != "0"),
                       "cron presente" if os.popen("crontab -l 2>/dev/null | grep -c sync-dms-to-gbrain").read().strip() != "0" else "MISSING en crontab")),
        Claim("§3/§4", "Cynthia/Daniel/Nati todos monitor-silent (no responden a nada)",
              "grep monitor-silent en cada perfil",
              lambda: (all(file_exists(f"~/.hermes/whatsapp/contacts/+{p}.md") and
