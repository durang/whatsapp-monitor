...
// sentMessageIds not needed — recentlySentIds (line ~158) already tracks echo-backs
...
async function startSocket() {
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // PATCH: In shared-number mode (same phone number), the owner's messages
      // are instructions to Hermes — in DMs AND in groups.
      // Only skip: status broadcasts and known bot echo-backs.
      if (msg.key.fromMe) {
        if (chatId.includes('status')) continue;
        // Skip echo-backs: if the message ID was sent by OUR bridge, skip it.
        if (recentlySentIds.has(msg.key.id)) {
          continue;
        }
        // All other fromMe messages (DM or group) = owner typing → allow through
      }

      // Handle !fromMe messages (from other people)
      if (!msg.key.fromMe) {
        // Always check allowlist for non-self messages
      ...
      // Skip Hermes' own reply messages to avoid loops.
